...
summarize_location <- function(.x, no_parentheses = FALSE){
  y = study1_participants %>% 
    filter(
      get('lugar de grabación') %in% .x) %>% 
    summarise(n = n(), n_men = sum(género == 'masculino'))
  
  if (no_parentheses){
    y %>% glue::glue_data("{n}, {n_men} men")
  }else{
    y %>% glue::glue_data("({n}, {n_men} men)")
  }
    }
...
We recruited `r pull(summarise(study1_participants, n(), .groups = 'drop'))` adult participants (`r pull(summarise(study1_participants, sum(género == 'masculino'), .groups = 'drop'))` men) from multiple sites ranging from more urban and industrialized (from Yarinacocha or San Francisco; `r summarize_location(c("bena jema", "hotel gauilanes", "san francisco"), T)`) to more traditional and in closer proximity to the surrounding rainforest (Nueva Betania, Paoyhan, or Puerto Belén; `r summarize_location(c("bilingue", "betania"), T)`). All villages were integrated in Peruvian economy and society. Given our relatively small sample size, we did not find apparent differences between participants from different sites.

We experienced difficulty recruiting male participants as many of the men were away from the village during the day, resulting in a sample that was predominantly female. Most participants `r summarize_location(c("betania", "san francisco", "bena jema", "hotel gauilanes"))` were from SK villages of the Middle Ucayali region (Yarinacocha, San Francisco, and Nueva Betania), with a subset from communities of the Lower (Paoyhan) and Upper (Puerto Belén) Ucayali regions. Within the small town of Yarinacocha (in the vicinity of Pucallpa), we recruited participants `r summarize_location(c("bena jema", "hotel gauilanes"))` from Bena Jema, a predominantly SK neighborhood. The remaining recruitment sites `r summarize_location(c("bilingue"))` were native community villages with exclusively SK residents but a strong relationship with those outside their community.
...
```{r study1-figure, fig.cap = "(A and B) Plots of the modal term given for a particular chip. Color coordinates were represented in 2-D Munsell space, with Munsell hue represented on the x-axis and Munsell value or lightness represented on the Y-axis. Modal responses were given by SK adults during (A) our Study 1 and during (B) the original World Color Survey. (C) Heat map of prevalence of Spanish-language responses during Study 1. Legends for all three subplots located in the bottom-right quadrant, with the upper legend corresponding to Plots A and B; the lower legend corresponds to plot C."}
...
<!-- TODO: Add binomial confidence intervals--> 

```{r study2-task-compare-plot, fig.cap = "Production and comprehension data for selected color chips, plotted by age group. Points show the raw average for a 2-year age group. Lines and error bars represent the 95% confidence intervals"}
...
study2_task_compare_plot <- study2_task_compare  %>%
    ...
    summarise(
        prop_correct = mean(score),
        n_total = n(),
        n_correct = sum(score),
        .groups = 'drop') %>% 
  group_by(task, prompt) %>% 
  group_split() %>% 
  map_dfr(~{.x %>% 
      mutate(binom = binom::binom.bayes(.x$n_correct, .x$n_total)) %>% 
      unnest(binom) %>% 
      select(-c("x","n", "method","shape1","shape2"))}) %>%
  ggplot(., aes(
    x = age_interval, y = mean, group = task, 
    col = task, fill=task, ymin=lower,ymax=upper)) +
  geom_linerange(position = position_dodge(width = .2)) +
  geom_line(size=1.2, position = position_dodge(width = .2)) + 
  geom_point(aes(y=prop_correct)) +
  facet_wrap(~ prompt, scales = "free") +
  ggthemes::scale_color_ptol() +
  ggthemes::scale_fill_ptol() +
  scale_x_continuous(limits = c(4.5,11.5), breaks = 5:11) +
  scale_y_continuous(limits = c(-0.1,1.1), breaks = seq(0,1,0.5)) +
  labs(x = "Age", y = "Proportion of prop_correct Responses", colour = "Task") +
  papaja::theme_apa() +
