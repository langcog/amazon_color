...
study2_prod_data <- feather::read_feather("../intermediates/processed_data.csv") %>%
...
```{r study2-demographics, results='asis'}
...
```{r study2-accuracyspanishentropy, results='asis'}
...
study2_spanishbyentropy <- study2_prod_data %>%
...
study2_spanishbyentropy$age.years.c <- scale(study2_spanishbyentropy$age, scale = FALSE)[,1]

study2_SwitchEnt_model <- lme4::glmer(
  spanish_response ~ age.years.c * Entropy + (1|subj),
  data=study2_spanishbyentropy, family = "binomial")
...
```{r study3-demographics, results = 'asis'}
study3_demographics <- study3_data %>%
  group_by(subj) %>%
  filter(row_number() == 1) %>% group_by(age_interval) %>%
  summarise(num_total = n(),
            num_boys = sum(gender == 'masculino'),
...
study3_switchperc <- study3_data %>%
  group_by(subj) %>%
  summarise(shipibo_response = 100*mean(shipibo_response), .groups = 'drop') %>%
...
study3_overallswitch <- study3_data %>%
  group_by(subj, age) %>%
  summarise(shipibo_response = mean(shipibo_response), .groups = 'drop')
