...
However, over a quarter (`r study2_spanishperc$spanish_response`) of all responses were given in Spanish. The distribution of Spanish responses was non-random. Children tended to respond in Spanish when presented with a chip which had low naming consensus (high entropy) among adults in Study 1. Even when accounting for individual differences in age, this effect remained strong (*p* `r with(study2_SpanEnt_model, ifelse(coefficients["Entropy", "Pr(>|z|)"] < 0.001, "< 0.001", paste0("= ", round(coefficients["Entropy", "Pr(>|z|)"], 3))))`, see inset entropy values in Figure 2).

Overextensions.
```{r study2_overextensions, include=FALSE}
study2_adjacentorswitch <- study2_data %>%
  ...
  summarise(correct = mean(correct), correcteither = mean(correct_either), correctnearby = mean(correct_nearby)) %>%
...
# study2_wrongresponsetypes <- study2_data %>%
#   group_by(subj, prompt) %>%
#   filter(first(correct) == 0 & response_order %in% c("first", "only", "last")) %>%
#   separate_rows(nature, sep = "-") %>%
#   mutate(nature = trimws(ifelse(is.na(nature), "Other", nature))) %>%
#   group_by(subj, age, response_order, nature) %>%
#   summarise(n = n()) %>%
...
#   mutate(prop = n/sum(n))
# 
# study2_wrongresponseloess <- bind_rows(lapply(
#   study2_wrongresponsetypes %>% split(list(.$response_order, .$nature)),
#   function(x){
#     if (nrow(x) > 6) {
#     as_data_frame(predict(loess(formula = prop ~ age, data = x, span = 0.75), 
#             newdata = predicted_range, se = TRUE)[c("fit", "se.fit")]) %>%
...
#     }}))
#   
# ggplot(study2_wrongresponseloess,
#   aes(x = predicted_age, y = fit, color = nature, group = nature)) + 
#   facet_grid(. ~ response_order) +
#   geom_line(size = 1) + geom_point(size = 4) +
#   geom_ribbon(aes(ymin = fit - se.fit, ymax = fit + se.fit, group = nature, fill = nature), alpha = .2, colour=NA) +
...
Similar to Study 2, over a quarter of all responses (`r study3_spanishperc`) were given in another language (Shipibo in this case). However, there was no significant correlation between age and label accuracy (*p* `r with(study3_overallscore_test, ifelse(p.value < 0.001, "< 0.001", paste0("= ", round(p.value, 3))))`). Children tended to respond in Shipibo when presented with items that had low entropy among SK adults during Study 1 (p `r with(study3_SpanEnt_model, ifelse(coefficients["Entropy", "Pr(>|z|)"] < 0.001, "< 0.001", paste0("= ", round(coefficients["Entropy", "Pr(>|z|)"], 3))))`). 

Overextensions.
```{r study3_overextensions, include=FALSE}

study3_adjacentorswitch <- study3_data %>%
  group_by(subj, age, gender) %>%
  filter(response_order %in% c("first", "only")) %>%
  mutate(correct_nearby = correct + ifelse(is.na(wrong_adjacent), 0, wrong_adjacent)) %>%
  summarise(correct = mean(correct), correcteither = mean(correct_either), correctnearby = mean(correct_nearby)) %>%
