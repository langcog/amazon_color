...
study2_overallswitch_test <- cor.test(study2_overallswitch$spanish_response, study2_overallswitch$age)
...
study2_EucDistModel <- lme4::lmer(
  wrong_euc_dist ~ age.years.c + (1|subj),
  data=study2_EucDist, REML=FALSE)

study2_EucDistModel_p <- pchisq(deviance(lm(wrong_euc_dist ~ 1, data=study2_EucDist)) - deviance(study2_EucDistModel), 78, lower.tail=FALSE)
...
study2_allresponsetypes <- study2_data %>%
  ...
  separate_rows(nature, sep = "-") %>%
  mutate(nature = trimws(ifelse(is.na(nature), "Other", nature))) %>%
  group_by(subj, age, nature) %>%
  summarise(n = n()) %>%
  group_by(subj, age) %>%
  mutate(prop = n/sum(n)) %>% ungroup() %>%
  complete(nesting(subj, age), nature, fill = list(n = 0, prop = 0.0)) %>%
  mutate(nature = factor(nature))
...
study3_overallswitch_test <- cor.test(study3_overallswitch$shipibo_response, study3_overallswitch$age)
...
study3_accuracyloess_plot <- ggplot(study3_accuracyloess,
  aes(x = predicted_age, y = fit, color = measure, group = measure)) + 
  geom_line(size = 1) + geom_point(size = 4) +
  geom_ribbon(aes(ymin = fit - se.fit, ymax = fit + se.fit, group = measure, fill = measure), alpha = .2, colour=NA) +
  scale_color_viridis(discrete=TRUE, option = "D") +
...
study3_responsedev <- deviance(multinom(Y~1, data=study3_allresponsetypes_whole)) - deviance(multinom(Y ~ age.years.c * response_order, data=study3_allresponsetypes_whole))

pchisq(study3_responsedev, 39+39-2, lower.tail = T)

```


Comparisons between Studies 2 & 3.
Proportion of ad hoc terms used in production tasks (compare across Studies 2 & 3)
- first response only (basic, ad hoc, non-ad hoc, spanish)
- do kids ever use a basic term at all in their responses?
...
studies2n3_firstresponse_whole <- studies2n3_data %>% ungroup() %>%
  filter(response_order %in% c("first", "only")) %>%
  select(language, subj, age, prompt, nature, spanish_response, shipibo_response) 

studies2n3_firstresponse <- studies2n3_firstresponse_whole %>%
  group_by(language, subj, age, nature, spanish_response, shipibo_response) %>%
  summarise(n = n()) %>% 
  group_by(language, subj, age) %>%
  mutate(prop = n/sum(n),
...
studies2n3_firstresploess <- bind_rows(lapply(
  studies2n3_firstresponse %>% 
    ...
    split(list(.$language, .$nature, .$response_language)),
  function(x){
    if (nrow(x) > 6) {
    as_data_frame(predict(loess(formula = prop ~ age, data = x, span = 0.75), 
            newdata = predicted_range, se = TRUE)[c("fit", "se.fit")]) %>%
      mutate(predicted_age = predicted_range,
             response_language = first(x$response_language),
             study_language = first(x$language),
             nature = case_when(
               first(x$nature) == "AHCT" ~ "Ad-hoc color term",
               first(x$nature) == "BCT" ~ "Basic color term",
               TRUE ~ "Other"))
...
studies2n3_firstresploess_plot <- ggplot(studies2n3_firstresploess,
  ...
  facet_grid(nature ~ study_language) +
  geom_line(size = 1) + geom_point(size = 4) +
