...
Study 1 - Gender differences in term profusion or term appearance
```{r, study1_genderdifferences}
...
```{r study2_accuracy, include=FALSE}
...
Older children displayed a higher level of overall accuracy in comparison to younger children (*r*(`r study2_overallscore_test$parameter`) = `r round(study2_overallscore_test$estimate, 3)`, *p* `r with(study2_overallscore_test, ifelse(p.value < 0.001, "< 0.001", paste0("= ", round(p.value, 3))))`, see Figure 2). 


Accuracy analyses and Spanish-language responses.
```{r study2_spanishentropy, include=FALSE}
...
However, over a quarter (`r study2_spanishperc$spanish_response`) of all responses were given in Spanish. The distribution of Spanish responses was non-random. Children tended to respond in Spanish when presented with a chip which had low naming consensus (high entropy) among adults in Study 1. Even when accounting for individual differences in age, this effect remained strong (*p* `r with(study2_SwitchEnt_model, ifelse(coefficients["Entropy", "Pr(>|z|)"] < 0.001, "< 0.001", paste0("= ", round(coefficients["Entropy", "Pr(>|z|)"], 3))))`, see inset entropy values in Figure 2).
...
study2_msat <- multinom(Y ~ age.years.c + response_order,
                 data=study2_allresponsetypes_whole)


study2_responsedev <- deviance(multinom(Y~1, data=study2_allresponsetypes_whole)) - deviance(study2_msat)
...
ggplot(study2_allresponseloess,
...
ggplot(study2_FLTypes %>% gather(key = "measure", value = "prop", first_BCT:last_AHCT),
...
Similar to Study 2, over a quarter of all responses (`r study3_spanishperc`) were given in another language (Shipibo in this case). However, there was no significant correlation between age and label accuracy (*p* `r with(study3_overallscore_test, ifelse(p.value < 0.001, "< 0.001", paste0("= ", round(p.value, 3))))`). Children tended to respond in Shipibo when presented with items that had low entropy among SK adults during Study 1 (p `r with(study3_SwitchEnt_model, ifelse(coefficients["Entropy", "Pr(>|z|)"] < 0.001, "< 0.001", paste0("= ", round(coefficients["Entropy", "Pr(>|z|)"], 3))))`). 
...
study_3msat <- multinom(Y ~ age.years.c + response_order,
                 data=study3_allresponsetypes_whole)

study3_responsedev <- deviance(multinom(Y~1, data=study3_allresponsetypes_whole)) - deviance(study_3msat)
...
ggplot(study3_allresponseloess,
  aes(x = predicted_age, y = fit, color = nature, group = nature)) + 
  facet_grid(. ~ response_order) +
  geom_line(size = 1) + geom_point(size = 4) +
  geom_ribbon(aes(ymin = fit - se.fit, ymax = fit + se.fit, group = nature, fill = nature), alpha = .2, colour=NA) +
...
study3_FLTypes$Y <- as.matrix(study3_FLTypes[, c("first_BCT", "first_AHCT", "NBCT", "Other", "SLT")])
