...
cd <- child_data %>%
  ...
  select(language, género, subj, age, age_scaled, task, prompt, spanish, response_num, response, correct, correct_switch, adjacency, dist) %>%
  ...
  group_by_at(vars(language:spanish)) %>%
...
write_csv(cd, "intermediates/processed_data.csv", na = "")
