...
clean_child_data <- function(df, min_consensus = 10) {
  ...
  both_chip_sets <- full_join(select(spanish_chip_set, -code), 
    ...
    mutate(
      # shipibo = ifelse(chip_id == 234, "Pei/Xo/Yankon",
      #                       ifelse(chip_id == 291, "Yankon",
      #                              ifelse(chip_id == 1, "Nai/Yankon", shipibo))),
           both_langs = ifelse(!is.na(shipibo),
                               str_c(spanish, shipibo, sep = ", "), spanish))
  
  shipibo_boundary_c <- shipibo_boundary %>%
    mutate(`chip_id.y` = as.character(`chip_id.y`),
           `Color_1` = ifelse(`Color_1` %in% c("Pei", "Xo"), "Pei/Xo", 
                              ifelse(`Color_1` %in% c("Ami", "Poa"), "Ami/Poa", `Color_1`)),
           `Color_2` = ifelse(`Color_2` %in% c("Pei", "Xo"), "Pei/Xo", 
                              ifelse(`Color_2` %in% c("Ami", "Poa"), "Ami/Poa", `Color_2`))) %>%
    group_by(`Color_1`, `Color_2`) %>%
    summarise(sum_chips = sum(sum_chips, na.rm = T),
              adjacency = ifelse(sum(adjacency, na.rm = T) > 1, 1, sum(adjacency, na.rm = T)),
    ...
    if (first(df$Language) == "Shipibo") {
      ...
      df <- df %>%
        ...
        mutate(correct = ifelse(str_detect(spanish, str_replace_na(response)), 1, 0),
               ...
               correct_switch = ifelse(spanish == "Azul" & response == "Nai", 1, correct_switch)) %>%
...
if (refresh_data) {
  ...
  child_data <- bind_rows("Shipibo" = read_csv("data/Current_Data/shipibo_children_colors_participants.csv") %>%
    ...
    mutate(age = ifelse(is.na(age), as.numeric(as.character(edad)), as.numeric(as.character(age))),
           ...
           age_interval = round(age/age_int)*age_int) %>%
    ...
    map(clean_child_data, min_consensus)
...
```{r}
cd <- child_data
cd$age.years.c <- scale(cd$age, scale = FALSE)[,1]
cd %<>%
  select(Language, subj, task, prompt, response_num,spanish,correct, correct_switch) %>%
  mutate_if(is.character, factor)

# pbt_mod <- glmer(proportion ~ age.years.c * task.type + (task.type | subj) + (task.type | prompt), data=pbt,
#                  family = "binomial")
# 
# summary(pbt_mod)
# 
# ggplot(child_data, aes(x = age_interval, y = correct, group = task, colour = task)) +
# 
#   
# pchildes_mod <- glmer(proportion ~ age.years.c * task.type + in_vocab + (task.type | subj) + (task.type | prompt),
#                  data=precision.childes, family = "binomial")
```
