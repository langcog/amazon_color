...
cd %<>%
  select(Language, subj, task, prompt, spanish, response_num, response, correct, correct_switch) %>%
  mutate_if(is.character, factor) %>%
  group_by_at(vars(Language:spanish)) %>%
  summarise(correct = mean(correct, na.rm = T) )

pbt_mod <- glmer(proportion ~ age.years.c * task.type + (task.type | subj) + (task.type | prompt), data=cd,
                 family = "binomial")
